export function StepCircle({ number, color, line = 'right' }) {
  const circle = (
    <div className="relative w-[50px] h-[50px] shrink-0 flex items-center justify-center">
      ...
      />
      <span
        className="relative font-semibold text-xl leading-none"
        style={{ color }}
      >
        {number}
      </span>
  ...
  const isVertical = line === 'up' || line === 'down'

  const connectorWidth = isVertical ? 2 : 31
  const connectorHeight = isVertical ? 31 : 2
  ...
  if (line === 'down') return <div className="flex flex-col items-center">{circle}{connector}</div>
